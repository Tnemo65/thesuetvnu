...
class NullSpikeInjector(FaultInjector):
    name = "null_spike"
    ...
    def inject(
        ...
        null_ratio = float(config["severity_map"][severity])
        ...
        for batch_id in target_batches:
            ...
            if len(batch_indices) == 0:
                continue
            sample_size = max(1, int(round(len(batch_indices) * null_ratio)))
            chosen = rng.choice(batch_indices, size=sample_size, replace=False)
            dirty_df.loc[chosen, target_column] = pd.NA
        ...
        incident = self.build_incident(
            ...
            metadata={"target_column": target_column, "null_ratio": null_ratio, "modified_rows": modified_rows},
        )
